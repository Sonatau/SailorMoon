...
import { SearchCouresComponent } from 'src/app/shared/components/search-coures/search-coures.component';
...
export class CouresPage implements OnInit {
  ...
  async search(type) {
    ...
    const modal = await this.modalController.create({
      component: SearchCouresComponent,
      componentProps: {
        type: type
      }
    });
        ...
        buttons: [
          {
            text: '创建班课',
            handler: () => {
              this.router.navigateByUrl('/coures/create-coures');
            }
          },
          {
            text: '取消',
